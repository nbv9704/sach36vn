...
declare
  ...
  v_effective_odds numeric(14, 4) := 1;
...
begin
  ...
  loop
    ...
    if coalesce(v_updated_selection->>'status', 'Pending') = 'Pending' then
      ...
      v_has_cancelled := true;
    elsif v_updated_selection->>'status' = 'Won' then
      v_won_count := v_won_count + 1;
      v_effective_odds := v_effective_odds * coalesce(nullif(v_updated_selection->>'odds', '')::numeric, 1);
    end if;
  end loop;
  ...
  if v_has_lost then
    ...
    if v_won_count > 0 then
      v_final_status := 'Won';
      v_settled_payout := round(v_bet.stake * v_effective_odds, 2);
